      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 text-black">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-700 text-sm font-medium mb-1">Total Customers</p>
              <p className="text-3xl font-bold">{userList.length}</p>
              <p className="text-gray-700 text-xs mt-2">All registered customers</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 text-black">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-700 text-sm font-medium mb-1">Active Customers</p>
              <p className="text-3xl font-bold">{activeCount}</p>
              <p className="text-gray-700 text-xs mt-2">
                {userList.length > 0 ? ((activeCount / userList.length) * 100).toFixed(0) : 0}% of total
              </p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 text-black">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-700 text-sm font-medium mb-1">Inactive Customers</p>
              <p className="text-3xl font-bold">{inactiveCount}</p>
              <p className="text-gray-700 text-xs mt-2">
                {userList.length > 0 ? ((inactiveCount / userList.length) * 100).toFixed(0) : 0}% of total
              </p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 text-black">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-700 text-xs mt-2">From all customers</p>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-800">All Customers</h1>
              <p className="text-gray-500 mt-1.5 flex items-center gap-2">
                <span className="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                  {filteredUsers.length}
                </span>
                {statusFilter === 'all' ? 'Total' : statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)} Customers
              </p>
            </div>
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-gray-500 font-medium font-inter">Syncing User Database...</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredUsers.map(user => (
                  <tr key={user._id || user.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${user.status === 'active'
